...
pub async fn translate_to_shell_command(
    ...
    let mut prompt = if let Some(history) = conversation_history {
        format!(
            "You are an expert in shell commands for macOS/Linux systems. \
            Your task is to translate the following natural language request into a single, \
            executable shell command. The command should be safe and appropriate for the user's \
            operating system.\n\n\
            IMPORTANT RULES:\n\
            ...
            5. If the request is vague or could be dangerous, return a safe alternative\n\
            6. Use the conversation history to understand context (e.g., 'open it' means open the app mentioned before)\n\n\
            Recent conversation:\n\
            {}\n\n\
            New request: {}\n\
            Command:",
